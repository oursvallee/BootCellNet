...
# 4/25/2024 
# (1) Automatic identification of cluster of clumps by using pvclust
# (2) Calculation of "critical nodes"
##############################################################################
...
knnsmo <- function(data, umap, K=30, theta=1, file, progress=T){
  require(Matrix)
  M0 <- dim(data)[1]*1
  N0 <- dim(data)[2]*1 # cell number
...
bootcellnet <- function(data, N=5000, M=100, goi=NULL, 
                        directory, suffix=NULL, filenum=0,
  ...
  for(m in 1:M){
    ...
    # data.cs <- as(matrix(0, nrow=G0, ncol=N), "dgCMatrix")
    ...
    write.table(sd.aracne, paste0(directory,"/bootcellnet_", suffix, m+filenum,".txt"), quote=F, sep="\t")
...
#===================================
# clean up GRN
# 4/25/2024: p.adjust incorporated
cleanGRN <- function(aracne.mtx, node.pvals, edge.pvals, 
                     method="BY", node.cutoff=.0001, edge.cutoff=.0001){
  
  require(igraph)
  
  node.qvals <- p.adjust(node.pvals, method=method)
  edge.qvals <- p.adjust(edge.pvals, method=method)
  ...
  nodes.sig <- names(node.qvals)[node.qvals<node.cutoff] # take only significant nodes
  
  aracne.mtx.clean <- aracne.mtx
  aracne.mtx.clean[edge.qvals>edge.cutoff] <- 0 # remove non-significant edges
...
# 4/25/2024 changed
# method:
# "default", only one solution is returned
# "critical", critical nodes are returned, with a given nsol
# "whole", both critical and intermittent nodes are returned
# the latter two also return frequencies of appearance in nsol solutions.
# also returns node shapes/colors for plotting the graph
MDSGRN <- function(ar.graph, method="default", nsol=15){
  
  a <- ar.graph$graph.clean$graph
  
  # if method is default, force nsol to be 1
  if(method=="default"){
    nsol <- 1
  }
  
  mds.freq <- mds.ip(a, genes=TRUE, nsol)
  # this returns a vector of frequencies
  
  # if "critical", return only critical nodes as MDS
  if(method=="critical"){
    mds <- names(mds.freq[mds.freq==nsol])
    intm <- names(mds.freq[((mds.freq>0)*(mds.freq<nsol))>0])
  }
  # else, return all nodes appeared as MDS
  else if(method=="whole"){
    mds <- names(mds.freq[mds.freq>0])
    intm <- names(mds.freq[((mds.freq>0)*(mds.freq<nsol))>0])
  }
  else{
    mds <- names(mds.freq[mds.freq>0])
  ...
  node.shape[names(V(a)) %in% intm] <- "circle"
  
  node.color <- rep("white", vcount(ar.graph$graph.clean$graph))
  node.color[names(V(ar.graph$graph.clean$graph)) %in% mds] <- "red"
  node.color[names(V(ar.graph$graph.clean$graph)) %in% intm] <- "red"
  
  ar.graph$graph.clean[["MDS"]] <- mds
  ar.graph$graph.clean[["critical"]] <- setdiff(mds, intm)
  ar.graph$graph.clean[["intermittent"]] <- intm
  ar.graph$graph.clean[["MDS_freq"]] <- mds.freq
  ar.graph$graph.clean[["node_shape_MDS"]] <- node.shape
  ar.graph$graph.clean[["node_color_MDS"]] <- node.color
...
# 4/25/2024: modified to return a vector of numbers of appearance
mds.ip <- function(graph, genes=F, nsol=1){
  
  if(nsol>20){
    cat("nsol may be too large, better less than 20...\n")
    cat("somehow lpsolve tends to crash with greater nsol\n")
    return(NULL)
  }
  ...
  lpsol <- lp("min", f.obj, f.con, f.dir, f.rhs, int.vec=1:n, all.bin=T, num.bin.solns=nsol)
  
  if(genes){
    ngenes <- lpsol$x.count
    
    nmds <- rep(0, ngenes)
    names(nmds) <- colnames(lpsol$constraints)
    
    for(i in 1:nsol){
      start <- 1+ngenes*(i-1); end <- ngenes*i
      nmds <- nmds + lpsol$solution[start:end]
    }
    return(nmds)
  }
  else{
...
# using pvclust with a preset cutoff p value
# input the result of pvclust, and returns clusters
# two methods to determine the number of clusters have been implemented
# (1) take best ARI clustering. tends to have finer clustering result
# (2) take maximal of ARIs. tends to have coarse clustering result and is more robust
autoClust <- function(data, alpha=.95, max.only=F, min.clst=10, max.clst=30, method="maximal"){
  require(pvclust)
  
  #=======================================
  # first, extract clusters based on the pvclust result
  clsts <- pvpick(data, alpha=alpha, max.only=max.only)
  
  clsts.vec <- rep(0,length=length(data$hclust$labels))
  names(clsts.vec) <- data$hclust$labels
  
  for(i in 1:length(clsts$clusters)){
    
    # which cell clumps are we dealing with?
    curr.clumps <- clsts$clusters[[i]]
    
    for(j in curr.clumps){
      curr.clst <- clsts.vec[j]
      ...
      # if not, compare the current and the new, and assign SMALLER cluster
        ...
        size.curr <- length(clsts$clusters[[curr.clst]])
        size.new  <- length(clsts$clusters[[i]])
  ...
  # assign cluster numbers to singleton clumps
  k <- max(clsts.vec)+1
  for(i in (1:length(clsts.vec))[clsts.vec==0]){
    clsts.vec[i] <- k
    k <- k+1
  }
  
  #=======================================
  # second, compare the pvclust results to hclust cutree w/ different k
  # take k with "best" ARI to pvclust clusters
  
  clst.aris <- vector(length=max.clst)
  for(i in 2:max.clst){
    data.cut <- cutree(data$hclust, k=i)
    clst.aris[i] <- calc.ari(clsts.vec, data.cut)
  }
  
  n.clst <- min.clst
  if(method=="best"){
    n.clst <- which.max(clst.aris)
  }
  else{
    
    j <- min.clst
    while(j<=max.clst){
      currvec <- clst.aris[1:j]
      jmax <- which.max(currvec)
      if(jmax<j){
        if(jmax>=min.clst){
          n.clst <- jmax
          j <- max.clst+1
  ...
  clst.vec.result <- cutree(data$hclust, k=n.clst)
  return(list(clusters=clst.vec.result, ARIs=clst.aris, pvclusters=clsts.vec))
